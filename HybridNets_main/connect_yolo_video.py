...
from HybridNets_main.utils.utils import letterbox, scale_coords, postprocess, BBoxTransform, ClipBoxes, restricted_float, \
...
def hybridnets_video(
    ...
    for seg_class in params.seg_list:
        ...
        color_list_seg[seg_class] = list(np.random.choice(range(256), size=3))#seg색상
    compound_coef = c
    source = source1
    if source.endswith("/"):#source의 값이 /로 끝나면 /를 지움
        source = source[:-1]
    output = output1
    if output.endswith("/"):#output의 값이 /로 끝나면 /를 지움
        output = output[:-1]
    weight = w
    if source.endswith('.mp4'):
        video_srcs = glob(f'{source}')#source로 지정한 .mp4파일의 이름을 리스트로 저장한다(이 부분때문에 source를 폴더명으로 주어야한다.)
        print('========>video_srcs: ',video_srcs)
    else:
        video_srcs = glob(f'{source}/*.mp4')#source폴더안에 있는 mp4파일들의 이름을 리스트로 저장한다(이 부분때문에 source를 폴더명으로 주어야한다.)
        if len(video_srcs)==0:
            print('경로를 확인해 주세요.')
        print('========>video_srcs: ', video_srcs)
    os.makedirs(output, exist_ok=True)#output경로에 폴더가 없으면 폴더를 만든다.
    input_imgs = []
    shapes = []
    ...
    clipBoxes = ClipBoxes()
#=======================================================================================================================
    for video_index, video_src in enumerate(video_srcs):#source경로안의 video수 만큼 돈다.
