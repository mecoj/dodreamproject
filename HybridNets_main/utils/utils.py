...
def letterbox(combination, new_shape=(384, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    ...
    """이미지의 상단과 하단에 회색 테두리를 확장하고 추가합니다.，참조：https://zhuanlan.zhihu.com/p/172121380"""
    # 이미지를 32픽셀 다중 사각형으로 크기 조정 https://github.com/ultralytics/yolov3/issues/232
    img, seg = combination#(input_img, None)
    shape = img.shape[:2]  # 현재 모양 [높이, 너비](360,640)
    print('=============>shape: ', shape)
    print('=============>new_shape: ', new_shape)
    if isinstance(new_shape, int):#new_shape이 int형이면 True를 반환
        new_shape = (new_shape, new_shape)#정사각형 모양의 shape ex) (640, 640)

    # Scale ratio (new / old)
    r = min(new_shape[0] / shape[0], new_shape[1] / shape[1])
    print('=============>r1: ', r)
    if not scaleup:  # 스케일 다운만 하고 스케일 업은 하지 않음(더 나은 테스트 mAP를 위해)
        r = min(r, 1.0)
    print('=============>r2: ', r)
    # Compute padding
    ...
    print('=============>dw: ',dw)
    print('=============>dh: ', dh)
    if auto:  # minimum rectangle /True
        dw, dh = np.mod(dw, 32), np.mod(dh, 32)  # wh padding/ mod()함수는 나머지값 반환
    elif scaleFill:  # stretch /False
        dw, dh = 0.0, 0.0
        new_unpad = (new_shape[1], new_shape[0])
        ratio = new_shape[1] / shape[1], new_shape[0] / shape[0]  # 너비, 높이 비율
